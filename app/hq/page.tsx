...
import { getTodayItems, getCompletedItems } from "../mission-control/progress-log";
import { TEMPLATES } from "../mission-control/template-registry";
import { FEATURES } from "../command-center/feature/features-data";
import ProgressCard from "../mission-control/components/ProgressCard";
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "40px 24px 80px" }}>
      ...
      {tab === "build" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
          ...
          {/* Templates */}
          <div>
            <SectionHeader title="Templates" count={productionTemplates.length} accent="#D4863E" />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 8 }}>
              {TEMPLATES.map((t) => (
                <a key={t.id} href={t.demoPath} target="_blank" rel="noopener noreferrer" style={{
                  display: "flex", alignItems: "center", gap: 10, padding: "10px 14px",
                  background: "#141420", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8,
                ...
                >
                  <div style={{ display: "flex", gap: 3 }}>
                    <div style={{ width: 12, height: 12, borderRadius: 3, background: t.colors.bg, border: "1px solid rgba(255,255,255,0.15)" }} />
                    <div style={{ width: 12, height: 12, borderRadius: 3, background: t.colors.accent }} />
                  </div>
                  <span style={{ fontSize: 12, color: "#ccc", fontWeight: 500 }}>{t.name}</span>
                  <span style={{ fontSize: 10, color: t.status === "production" ? "#4ade80" : "#666", marginLeft: "auto" }}>
                    {t.status === "production" ? "Live" : t.status}
                  </span>
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 8 }}>
              <LinkCard label="Mission Control" description="Full template registry + progress log" href="/mission-control" accent="#22c55e" />
              ...
              <LinkCard label="Workflows" description="Automation workflows to build" href="/mission-control?tab=workflows" accent="#22c55e" />
          ...
          <div>
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#888", lineHeight: 1.5 }}>
                ...
                Filter by relevance (High/Medium/Low) and topic.
              </div>
            </a>
          </div>
