...
export function RoofRender3D({ scene }: { scene: RoofScene }) {
  ...
  const fit = useMemo(() => {
    const w = Math.max(1, scene.bounds.maxX - scene.bounds.minX);
    const d = Math.max(1, scene.bounds.maxY - scene.bounds.minY);
    const span = Math.max(w, d);
    const dist = span * 1.6 + 20;
    return {
      camPos: [dist, dist * 0.8, dist] as [number, number, number],
      minD: dist * 0.5,
      maxD: dist * 3,
    };
  ...
  return (
    ...
    >
      <Canvas
        key={`${fit.camPos[0]}-${fit.camPos[1]}`}
        camera={{ position: fit.camPos, fov: 32 }}
      ...
      >
        <Suspense fallback={null}>
          ...
          <OrbitControls
            enablePan={false}
            minDistance={fit.minD}
            maxDistance={fit.maxD}
