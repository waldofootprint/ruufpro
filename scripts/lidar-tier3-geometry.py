...
# --- PERIMETER via alpha-shape (design §5) ---
def alpha_shape_perimeter(points_2d):
    """Auto-alpha = 2 * sqrt(area/N). Returns (perimeter_ft, horiz_area_sqft, flags)."""
    flags = {}
    if len(points_2d) < 4:
    ...
    alpha = 2.0 * s
    # Compute alpha-shape via Delaunay triangulation + edge filtering
    ...
    # Count edges ONLY across alpha-passing triangles. Boundary = edges seen once in the alpha-complex.
    alpha_edge_count = {}
    area_sum = 0.0
    kept_triangles = 0
    for simplex in tri.simplices:
        ...
        if cr is None or cr > alpha: continue
        kept_triangles += 1
    ...
    # Robustness check (§5.2): if alpha-area < 0.7 * hull-area, fall back
